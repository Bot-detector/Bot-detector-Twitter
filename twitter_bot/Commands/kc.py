...
def kc(rsn):
    ...
    data = web_request(
        "https://www.osrsbotdetector.com/dev/site/dashboard/projectstats"
    )
    project_ban_pct = 11
    ...
    data = web_request(
        f"https://www.osrsbotdetector.com/dev/v1/stats/contributions/{rsn}"
    )
    total = data["total"]
    manual = data["manual"]
    bans, reports, manual_reports, manual_incorrect = (
        total["bans"],
        total["reports"],
        manual["reports"],
        manual["incorrect_reports"],
    )
    ...
    if manual_reports != 0:
        accuracy = round(
            (manual_reports - manual_incorrect) / (manual_reports) * 100, 2
        )
